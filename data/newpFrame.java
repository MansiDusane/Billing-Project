...
public class newpFrame extends JFrame implements ComponentListener
{
	...
	public void componentHidden(ComponentEvent e) {}
			...
			try{
					...
					Connection connm = DriverManager.getConnection("jdbc:mysql://localhost:3306/billmanagementproject","root","");
					Statement stmtm = connm.createStatement();
					ResultSet rs = stmtm.executeQuery(" select max(p_id) from product ");
					if(rs.first())
					{
					int	id = rs.getInt(1);
					id = id + 1;
					...
					try
						{
							...
							connp = DriverManager.getConnection("jdbc:mysql://localhost:3306/billmanagementproject","Mansi","mansi@123");
